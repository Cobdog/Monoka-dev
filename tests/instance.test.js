...
maybe('(c) live pack detection from object_info', () => {
  console.log('instance: live pack detection from object_info')
  {
    ...
    ok(nodePackInstanceState(motionContext, ['MiniMaxH3MotionContext']) === 'absent', 'R5: a PARTIAL Motion-Context serving (one of four classes) reads absent on the board — the chain lane loads all four, so "active" would tell the user a broken install needs nothing')
    ok(nodePackInstanceState(motionContext, ['MiniMaxH3MotionContext', 'MiniMaxH3MotionContextTrim', 'MiniMaxH3MotionContextSaveLatent', 'MiniMaxH3MotionContextLoadLatent']) === 'active' && nodePackInstanceState(motionContext, ['KSamplerSelect']) === 'absent', 'R5/GAP-1: the full class set reads active; an answering instance without them reads absent')
    const turboPack = ENGINE_NODE_PACKS.find((entry) => entry.id === 'minimax-h3-turbo')
    ok(nodePackInstanceState(turboPack, ['MiniMaxH3TurboLoRA']) === 'absent' && nodePackInstanceState(turboPack, ['MiniMaxH3TurboLoRA', 'MiniMaxH3TurboSampler']) === 'active', 'R5: the turbo pack board verdict finally AGREES with larryvrhTurboPackPresent — the dedicated pair is all-or-nothing on both surfaces')
